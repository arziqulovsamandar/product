<template>
  <div
    class="transition-all justify-between duration-300 relative card"
    :class="cardWidth"
  >
...
import { writeToLocaleStorage, checkFromLocaleStorage } from '../../../hooks/localeStorage.js';
...
function addDataToCart() {
  addedCart.value = writeToLocaleStorage('myProduct', cartStore, props.product);
}

onMounted(() => {
  addedCart.value = checkFromLocaleStorage('myProduct', props.product);
});

const cardWidth = ref('');

onMounted(() => {
  cardWidth.value = calculateCardWidth();
});

function calculateCardWidth() {
  return '2lg:w-[calc(20%-10px)] 3xs:w-[calc(50%-10px)] md:w-[calc(100%/3-10px)] w-full';
}
