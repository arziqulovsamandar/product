...
const getProducts = async () => {
  ...
  try {
    if (route.query.featured) {
      ...
      const response = await axios.get('/api/dailyBestSells');
      data = response.data;
...
watch(() => useRoute().query, getProducts);
onMounted(getProducts);

const productsCount = computed(() => productsList.value.length);
const resultsPerPage = ref(10);
const currentPage = ref(1);
const numberOfPages = computed(() => Math.ceil(productsCount.value / resultsPerPage.value));
const nextPage = computed(() => currentPage.value + 1);
const previousPage = computed(() => currentPage.value - 1);
const startIndex = computed(() => (currentPage.value - 1) * resultsPerPage.value);
const endIndex = computed(() => currentPage.value * resultsPerPage.value);

const showFilters = [resultsPerPage.value, resultsPerPage.value + 10, resultsPerPage.value + 20];
      ...
      <div class="products">
        ...
        <div v-if="!isLoading" class="summary">
          ...
          <p class="text-gray-200">
            Found
            <span class="font-medium text-green-200">{{ productsCount }}</span>
            Items!
          </p>

          <!-- Controls -->
          <div class="flex flex-col gap-2 2xs:flex-row">
            <BaseDropdown :values="showFilters" class="basis-full" @value-changed="updateResultsPerPage">
              <template #icon>
            ...
            <BaseDropdown :values="sortFilters" class="basis-full" @value-changed="sortResults">
        ...
        <div v-if="!isLoading" class="products-grid">
          ...
          <div class="pagination-controls">
            <button v-if="previousPage" aria-label="Previous Page" @click="currentPage--">
            ...
            <button v-if="nextPage <= numberOfPages" aria-label="Next Page" @click="currentPage++">
